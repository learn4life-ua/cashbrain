...
function getEnding() {
  ...
  if (state.unpaidHours >= 5 && state.fraudLoss < 900 && !state.exposedData) {
    return {
  ...
  if (state.fraudLoss >= 900 || state.exposedData || state.muleRisk || state.risk >= 80) {
    return {
      ...
      title:"Обіцянка доходу відкрила доступ до ваших ресурсів",
      lead:"Замість роботи рекрутер отримав гроші, дані, безоплатну працю або доступ до вашого рахунку. Варто негайно припинити контакт, захистити банківські дані й зберегти докази."
    };
  }
  return {
