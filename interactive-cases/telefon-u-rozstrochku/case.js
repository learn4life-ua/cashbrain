...
const safeFinalStage = {
  ...
  choices: [
    ...
    {
      label: "За чотири місяці накопичити й купити перевірений уживаний телефон за 9 000 грн",
      hint: "Відкладати гроші окремо від резерву та перевірити стан, документи й блокування.",
      ...
      outcome: "За чотири місяці ви відкладаєте 9 000 грн із вільного бюджету, не витрачаючи резерв. Покупка не створює регулярного платежу, а діагностика й перевірка документів зменшують ризик уживаної техніки.",
      strength: "На дешевшу альтернативу накопичено окремо від резерву й без нового боргу."
    },
    {
...
function getEnding() {
  ...
  if (state.debt > 0 && state.phonePayment > 0) {
    return {
  ...
  if (state.debt > 0) {
    return {
      badge:"Терміновий кредит",
      title:"Ремонт профінансовано дорогим коротким боргом",
      lead:"Телефон ви не купили в кредит, але за ремонт ноутбука доведеться повернути 5 400 грн замість 4 000 грн. Це один платіж, а не два, проте він зменшує вільний бюджет на шість місяців."
    };
  }
  if (payment > state.freeMonthly || state.risk >= 80) {
    return {
      badge:"Небезпечне рішення",
      title:"Фінансове навантаження перевищило запас міцності",
      lead:"Проблема не обов’язково у двох кредитах. Рішення стало ризикованим через надто великий платіж, поспіх або майже порожній резерв."
    };
  }
  if (state.phoneCost >= 30000) {
    return {
      badge:"Дорога зручність",
      title:state.informed ? "Ви побачили переплату, але все одно погодилися на неї" : "Місячний платіж приховав справжню ціну телефона",
      lead:state.informed
        ? "Паспорт кредиту й розрахунок показали повну вартість, тому рішення було поінформованим, але дорогим. Загальна сума все одно значно перевищує ціну готівкою."
        : "Покупка відбулася, але загальна сума значно перевищила ціну готівкою. Головний урок - завжди рахувати весь строк і перевіряти додаткові послуги."
    };
  }
