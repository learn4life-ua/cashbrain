const initialState = {
  ...
  serviceIntakeDocumented: false,
  repairDocumented: false,
  usedPurchaseChecked: false,
  strengths: [],
  improvements: []
};

function spendReserve(state, amount) {
  const uncovered = Math.max(0, amount - state.reserve);
  state.reserve = Math.max(0, state.reserve - amount);
  state.debt += uncovered;
}
...
const stages = [
  {
    ...
    choices: [
      ...
      },
      {
        ...
        apply:s=>{spendReserve(s,500);s.cost+=500;s.diagnosisCompleted=true;s.risk-=12;s.hours-=6;},
  ...
  },
  {
    ...
    choices: [
      ...
      },
      {
        ...
        apply:s=>{s.serviceIntakeDocumented=true;s.risk-=15;s.hours-=2;},
  ...
  },
  {
    ...
    choices: [
      ...
      },
      {
        ...
        apply:s=>{s.assignmentSafe=true;spendReserve(s,700);s.cost+=700;s.risk-=5;s.hours-=3;},
  ...
  },
  {
    ...
    copy: "Після оцінки ситуації є три варіанти: ремонт за 5 500 грн, уживаний ноутбук за 8 500 грн із можливістю перевірки або новий за 30 000 грн у кредит із платежем 2 500 грн протягом року.",
    choices: [
      {
        label: "Ремонтувати за 5 500 грн",
        hint: "Сервіс дає кошторис і три місяці гарантії на роботу.",
        apply:s=>{s.plan="repair";spendReserve(s,5500);s.cost+=5500;s.risk-=8;},
      ...
      },
      {
        ...
        apply:s=>{s.plan="used";spendReserve(s,8500);s.cost+=8500;s.risk+=8;},
      ...
      },
      {
        ...
        apply:s=>{s.plan="new";s.payment=2500;s.cost+=30000;spendReserve(s,2500);s.risk+=25;},
...
function ending(){
 ...
 if(state.debt>0)return{badge:"Дефіцит коштів",title:"Обране рішення виявилося дорожчим за наявний резерв",lead:`Резерв закінчився, а ще ${money(state.debt)} залишилися без визначеного джерела. Потрібно окремо вирішити, звідки взяти цю суму, або обрати дешевший варіант.`};
 ...
 if(state.plan==="used"&&state.usedPurchaseChecked&&state.risk<=40)return{badge:"Перевірена альтернатива",title:"Уживаний ноутбук став контрольованою покупкою",lead:"Ви не поклалися лише на низьку ціну: пристрій перевірено, а покупку й комплектацію зафіксовано."};
 if(state.plan==="repair"&&state.diagnosisCompleted&&state.serviceIntakeDocumented&&state.repairDocumented&&state.risk<=35)return{badge:"Стійке рішення",title:"Курсова завершена, ноутбук ремонтується, бюджет збережено",lead:"Ви відокремили термінове завдання від ремонту, отримали діагностику, захистили дані та окремо зафіксували приймання техніки й умови ремонту."};
